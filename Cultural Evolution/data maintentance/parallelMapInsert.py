...
# note that this function only works if func (the function that is mapped over each element in in_collection) returns objects with _id fields, because this is how it checks to see if an object already exists in out_collection
	...
	if updateFreq > 1:
		...
		def partFunc(cursor):
			...
			for item in cursor:
				...
				out = func(item)
                # IT IS REQUIRED THAT OUT HAS AN ID FIELD
				if out:
